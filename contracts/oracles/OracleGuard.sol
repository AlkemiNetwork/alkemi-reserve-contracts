...
contract OracleGuard {
   ...
   */
    modifier auth(uint256 permission) {
        require(auths[msg.sender] == permission, "OracleGuard: not authorized");
        _;
    }
   ...
   */
    function isNodeAuth(address _node) external view returns (bool) {
        return ((nodes[_node] == 1) &&
            (slot[uint8(uint256(_node) >> 152)] == _node));
    }
   ...
   */
    function isNodeBan(address _node) external view returns (bool) {
        return ((nodes[_node] == 2) &&
            (slot[uint8(uint256(_node) >> 152)] == address(0)));
    }
   ...
   */
    function rely(address usr, uint256 permission) external auth(1) {
   ...
   * @param permission Permission type
   */
    function deny(address usr, uint256 permission) external auth(1) {
        auths[usr] = permission;
    }
   ...
   */
    function registerNode(address[] calldata a) external auth(1) {
        for (uint256 i = 0; i < a.length; i++) {
            ...
            require(
                slot[s] == address(0),
                "OracleGuard: oracle already authorized"
            );
            nodes[a[i]] = 1;
            slot[s] = a[i];
            nodesCounter++;
   ...
   */
    function authContract(address[] calldata a) external auth(1) {
        for (uint256 i = 0; i < a.length; i++) {
            require(
                a[i] != address(0),
                "OracleGuard: invalid contract address"
            );
            contracts[a[i]] = 1;
        }
    }
