...
contract LiquidityReserveState {
   ...
   */
    constructor(address alkemiNetwork, address liquidityProvider) internal {
        require(
            alkemiNetwork != address(0),
            "LiquidityReserveState: invalid liquidity manager contract address"
        );
        require(
            liquidityProvider != address(0),
            "LiquidityReserveState: invalid liquidity provider address"
        );
   ...
   */
    modifier onlyPermissioned() {
        require(
            havePermission(),
            "LiquidityReserveState: caller does not have permission"
        );
        _;
    }
   ...
   */
    modifier onlyLiquidityProvider() {
        require(
            isLiquidityprovider(),
            "LiquidityReserveState: caller is not the liquidity provider"
        );
        _;
    }
   ...
   */
    modifier onlyAlkemi() {
        require(
            isAlkemiNetwork(),
            "LiquidityReserveState: caller is not the Alkemi Network contract"
        );
        _;
    }
   ...
   */
    function transferLiquidityProvider(address newLiquidityprovider)
        public
   ...
   */
    function _transferLiquidityProvider(address newLiquidityprovider) internal {
        require(
            newLiquidityprovider != address(0),
